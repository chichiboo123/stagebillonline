...
### 2. 작품 색인 (사전형 보기)
카테고리가 아니라 **작품명**으로 전체 목록을 훑어보는 화면입니다. 내비 우측 📖 아이콘(모바일은 ⋯ 메뉴)으로 엽니다.

- 첫 글자로 자동 그룹핑 — 한글 **초성(ㄱㄴㄷ…)** / 알파벳 **A–Z** / 일본어 **오십음행(あかさ…)** / 한자(漢) / 기타(#)
- UI 언어에 따라 그룹 순서가 바뀝니다 (한국어: ㄱ→A→あ, 영어: A→ㄱ→あ, 일본어: あ→A→ㄱ)
- 상단 **점프바**로 해당 글자 위치로 즉시 이동
- **초성 검색** 지원 — `ㅂㄹ` 입력 → `빨래` (된소리는 예사소리로 합쳐 검색)
- 같은 제목의 여러 카테고리 버전은 한 줄로 묶고, 카테고리 칩으로 표시
...
- **버튼 통합**: `업로드` · `소개`는 모든 화면에서 우측 `⋯` 메뉴로 모았습니다. 좁은 데스크톱(≤1100px)에서는 `AI 큐레이션`이 아이콘만 표시됩니다
...
│  · 다국어, 검색, 색인, 모달  │         │  · handleAICuration (Gemini) │
...
```
stagebillonline/
├── index.html              # 단일 페이지 마크업 (네비/히어로/색인/모달/AI·업로드·소개 오버레이)
├── css/style.css           # 전체 스타일 (반응형, 카테고리 색상, 색인, 모달, AI 카드 등)
├── js/app.js               # 앱 로직 (데이터 로딩·검색·필터·색인·모달·AI 큐레이션·공유링크·다국어)
├── data/
